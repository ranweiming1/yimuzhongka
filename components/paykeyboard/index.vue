<template>
	<view>
		...
		<view :class="['content', show ? '' : 'contenthide']">
			<view class="title">
				<view class="close" @click="closed()"><image src="/static/close.png" /></view>
	...
	methods: {
		key(key) {
			if (this.password.length < 6) {
				this.password += key;
				if (this.password.length == 6) {
					this.$parent.InputComplete(this.password);
				}
			}
		},
		clear() {

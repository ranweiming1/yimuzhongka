<template>
	<view>
		...
		<view v-for='(item,index) in list' :id='item.name'>
			...
			<view v-for='(items,indexs) in item.list' :style='indexs==item.list.length-1?"overflow:hidden;line-height:90rpx;margin:0 20rpx;":"overflow:hidden;line-height:90rpx;margin:0 20rpx;border-bottom:1px dashed #efefef;"' @tap='qiehuan(items.carId,items.carName)'>
...
export default{
	...
	methods:{
		...
		qiehuan:function(carId,carName){
			...
			this.zhezhao=true
			this.pinpai=carName
			this.$https({url:'/api/shop/car-type-lists',data:{carId:carId},success:function(res){
